...
# NOTE: v0.2 is a custom version for this run. The goal is to find the slave barcodes that yields the best read recovery upon sample deconvolution (MIGEC checkout), this particularly for the beta chain seq. libraries from sample project BulkTCR023.
...
migec.path <- paste0(gen.seq.path, '/migec_read_recovery_exp')
if(!dir.exists(migec.path)) dir.create(migec.path)
jobs.path <- paste0(gen.seq.path, '/jobs_scripts/migec_read_recovery_exp')
...
if(!tmp.check) stop('Unexpected error!\n')
# CUSTOM PROCESS
# Identiy unique slave sequences used for beta chain libraries throughout whole seq. run.
tmp.cols <- c('slave.id', 'slave.seq')
beta.slave.bcs <- unique(bc.info[chain.type=='TCRB', ..tmp.cols])
beta.slave.bcs[, slave.lab:=str_replace(string=slave.id, pattern=' ', replacement='')]
# Keep only libraries of interest, i.e., beta chain libraries from sample project "BulkTCR023"
bc.info <- bc.info[sample.prj=='BULK_TCR23' & chain.type=='TCRB']
...
for(slave.bc.id in beta.slave.bcs[, slave.id]){
    # slave.bc.lab <- str_replace(string=slave.bc.id, pattern=' ', replacement='')
    slave.bc.lab <- beta.slave.bcs[slave.id==slave.bc.id, slave.lab]

    # ---> Predefinitions.
    tmp.path <- paste0(jobs.path, '/checkout_', (slave.bc.lab))
    ...
    tmp.data <- merge(x=tmp.data.1, y=tmp.data.2, by=c('sample.prj', 'seq.lib.name'), all.x=TRUE, all.y=FALSE, allow.cartesian=TRUE)
    ...
    if(!tmp.check) stop('Unexpected error!\n')
    # CUSTOM PROCESS
    # Replace beta chain slave sequences w/ the trial sequence.
    tmp.data$slave.seq <- beta.slave.bcs[slave.id==slave.bc.id, slave.seq]
    ...
    # )
    barcodes.file <- paste0(tmp.path, '/BarcodesFile_Checkout.tsv')
    ...
    run.job(
        ...
        line.rest=paste0(' -cute ', barcodes.file, ' ', migec.path, '/', basename(tmp.path)),
        memory=250
    )
}

# ---> Results exploration.
tmp.path <- paste0(migec.path, '/checkout_summ')
if(!dir.exists(tmp.path)) dir.create(tmp.path)
# Read results and preprocess.
tmp.file.names <- paste0(migec.path, '/checkout_', beta.slave.bcs[, slave.lab], '/checkout.log.txt')
names(tmp.file.names) <- beta.slave.bcs[, slave.lab]
tmp.check <- all(file.exists(tmp.file.names))
if(!tmp.check) stop('Could not find log files from checkout outs.\n')
tmp.data <- lapply(X=tmp.file.names, FUN=function(tmp.file.name){
...
tmp.data <- rbindlist(l=tmp.data, use.names=TRUE, idcol='slave.id')
...
tmp.data <- tmp.data[, .(slave.id, lane.no, seq.lib.name, tcr.lib.name, reads.w.master.bc, reads.w.both.bcs)]
# Sanity check. Total number of reads should greatly approximate the total number of reads from the deconvolution stats.
tmp.data.1 <- tmp.data[, .(migec.reads=sum(reads.w.master.bc)), by=.(slave.id, seq.lib.name, lane.no)]
...
# Final reports.
#       Per TCR library.
tmp.report <- as.data.table(spread(data=tmp.data, key='slave.id', value='reads.w.both.bcs'))
colnames(tmp.report)[
    colnames(tmp.report) %in% beta.slave.bcs[, slave.lab]
] <- paste0(
   'reads.w.both.bcs.',
    colnames(tmp.report)[colnames(tmp.report) %in% beta.slave.bcs[, slave.lab]]
)
tmp.file.name <- paste0(tmp.path, '/ReportPerTCRLibrary.csv')
...
tmp.data.1 <- tmp.data[, 
    ...
    by=.(slave.id, lane.no, seq.lib.name)
]
merge.cols <- c('lane.no', 'seq.lib.name')
key.col <- 'slave.id'
val.cols <- setdiff(x=colnames(tmp.data.1), y=c(key.col, merge.cols))
tmp.report <- lapply(X=val.cols, FUN=function(val.col){
    tmp.cols <- c(merge.cols, key.col, val.col)
    tmp.report <- tmp.data.1[, ..tmp.cols]
    tmp.report <- as.data.table(spread(data=tmp.report, key=key.col, value=val.col))
    colnames(tmp.report)[!colnames(tmp.report) %in% merge.cols] <- paste0(
        val.col, '.',
        colnames(tmp.report)[!colnames(tmp.report) %in% merge.cols]
    )
    return(tmp.report)
})
tmp.report <- Reduce(x=tmp.report, f=function(x, y){
    merge(x=x, y=y, by=merge.cols)
})
tmp.file.name <- paste0(tmp.path, '/ReportPerSeqLibrary.csv')
fwrite(file=tmp.file.name, x=tmp.report, na=NA)
